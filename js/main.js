...
class StakingSimulator {
    ...
    setupEventListeners() {
        ...
        this.inputs.xtz.addEventListener('input', (e) => handleNum(e, 1000000, 0));
        this.inputs.fee.addEventListener('input', (e) => handleNum(e, 100, 0));
        ...
        try {
            if (aggregatedDataCache?.homeData?.stakingData) {
                const {
                    stakingApy,
                    delegationApy
                } = aggregatedDataCache.homeData.stakingData;
    ...
    calculateRewards() {
        if (!this.inputs.xtz)
            return;
        const principal = parseInt(this.inputs.xtz.value) || 0;
        const days = parseInt(this.inputs.days.value) || 0;
        const bakerFee = parseInt(this.inputs.fee.value) || 0;
        if (principal <= 0) {
        ...
        this.updateDisplays(stakingRewards, principal + stakingRewards, delegationRewards, principal + delegationRewards);
    }
...
function setupSmartTooltipPositioning() {
    ...
    new MutationObserver(updateTooltipPositions).observe(bakersGrid, {
        childList: true
    });
}
function renderBakers(bakers) {
    ...
    const sorted = bakers.filter(b => b.status === 'active' && b.staking?.enabled).sort((a, b) => (b.staking?.estimatedApy || 0) - (a.staking?.estimatedApy || 0)).slice(0, 100);
    const batchSize = 10;
    let idx = 0;
...
function calculateAverageDifference(arr) {
    return arr.reduce((sum, val, idx, array) => idx > 0 ? sum + Math.abs(val - array[idx - 1]) : sum, 0) / (arr.length - 1);
}
...
async function initializeRatios() {
    ...
    try {
        ...
        while (ratios.length < relevantData.length + 800) {
...
function createVerticalLine(chart, xValue) {
    ...
    if (dataPoint) {
        ...
        chart.renderer.path(['M', xPos, yPosTop, 'L', xPos, yPosBottom]).attr({
            'stroke-width': 0.5,
            stroke: '#ffffff',
        }).add();
    }
        ...
        series: [{
                ...
                data: [{
                        ...
                        color: {
                            ...
                            stops: [[0, 'hsla(220, 85%, 70%, 0.9)'], [0.5, 'hsla(220, 75%, 55%, 0.8)'], [1, 'hsla(220, 65%, 40%, 0.7)']]
                        }
                    }, {
                        ...
                        color: {
                            ...
                            stops: [[0, 'hsla(280, 85%, 70%, 0.9)'], [0.5, 'hsla(280, 75%, 55%, 0.8)'], [1, 'hsla(280, 65%, 40%, 0.7)']]
                        }
                    }, {
                        name: 'Passive',
                        y: jeetsPercentage,
                        color: {
                            ...
                            stops: [[0, 'hsla(10, 85%, 70%, 0.9)'], [0.5, 'hsla(10, 75%, 55%, 0.8)'], [1, 'hsla(10, 65%, 40%, 0.7)']]
                        }
                    }
                ],
                showInLegend: false,
...
function createHistoricalTvlChart() {
    try {
        ...
        let rawData = aggregatedDataCache.combinedTvlChart.filter(item => item[0] > filterTimestamp).map(item => [item[0] / 1000, item[1]]).sort((a, b) => a[0] - b[0]);
...
function createHistoricalCharts(ratio) {
    try {
        ...
        const stakingDataWithRatio = [...stakingData.ratios, ...ratio.map((ratioValue, index) => ({
                    cycle: currentCycle + index + 1,
                    staking: 0,
                    ratio: ratioValue
                }))];
    ...
    const chartConfig = {
                        ...
                        [428, 743, 823, currentCycle].forEach(position => {
                            ...
                            if (dataPoint) {
                                ...
                                if (yPixel < bottomPixel) {
                                    this.renderer.path(['M', xPixel, bottomPixel, 'L', xPixel, yPixel]).attr({
                                        'stroke-width': 1,
                                        stroke: '#ffffff',
        ...
        series: [{
                ...
                color: {
                    ...
                    stops: containerId === 'issuanceh' ? [[0, '#ff6961'], [1, '#77dd77']] : [[0, '#77dd77'], [1, '#ff6961']]
                },
                name: title,
...
function processStakingData(data) {
    return {
        ratios: data.slice(1).map(curr => ({
                cycle: curr.cycle,
                staking: (curr.cycle >= 822 && (curr.totalOwnStaked || curr.totalExternalStaked)
                    ? curr.totalOwnStaked + curr.totalExternalStaked
                    : curr.totalFrozen) / curr.totalSupply
            })),
        currentCycle: data[data.length - 1].cycle
    };
}
        ...
        const pieData = projects.map((project, index) => {
            ...
            return {
                ...
                color: {
                    ...
                    stops: [[0, `hsla(${hue}, ${saturation}%, ${lightness + 15}%, 0.9)`], [0.5, `hsla(${hue}, ${saturation - 10}%, ${lightness}%, 0.8)`], [1, `hsla(${hue}, ${saturation - 20}%, ${lightness - 15}%, 0.7)`]]
                }
            };
        });
...
async function createEcosystemChart() {
    try {
        ...
        if (!projects || projects.length === 0) {
            ...
            document.getElementById('ecosystem-chart-container').innerHTML = '<div style="color: #ff6961; text-align: center; padding: 50px;">No ecosystem data available</div>';
            return;
        }
                ...
                if (point.custom && point.custom.logoUrl && point.graphic) {
                    ...
                    group.element.addEventListener('mouseenter', function (e) {
                        if (point.customBorderCircle) {
                            point.customBorderCircle.attr({
                                opacity: 1
                            });
                        }
                        const customTooltip = document.getElementById('custom-tooltip');
                        const logoHtml = point.custom.logoUrl ? `<img src="${escapeHTML(point.custom.logoUrl)}" style="width: 32px; height: 32px; border-radius: 8px; margin-bottom: 8px; background: white; padding: 2px;" />` : '';
                    ...
                    const bgCircle = chart.renderer.circle(plotX, plotY, circleSize / 2).attr({
                        fill: 'white'
                    }).add(group);
                    ...
                    chart.renderer.image(logoUrl, -imgSize / 2, -imgSize / 2, imgSize, imgSize).attr({
                        preserveAspectRatio: 'xMidYMid slice'
                    }).add(imgGroup);
                    const borderCircle = chart.renderer.circle(plotX, plotY, circleSize / 2).attr({
                        fill: 'none',
        ...
        function fillCenter(count, label, chart, customLabel) {
            ...
            `;
            if (!customLabel) {
                customLabel = chart.renderer.label(labelText, 0, 0, undefined, undefined, undefined, true).css({
                    color: '#ffffff',
                    pointerEvents: 'none'
                }).attr({
                    zIndex: 0
        ...
        const chartConfig = {
            chart: {
                ...
                events: {
                    ...
                    render() {
                        ...
                        if (pieSeries) {
                            pieSeries.customLabel = fillCenter(processedProjects.size, 'TOTAL PROJECTS', chart, pieSeries.customLabel);
                        }
                    }
                }
            },
            ...
            plotOptions: {
                ...
                bubble: {
                    minSize: window.innerWidth < 480 ? 6 : window.innerWidth < 640 ? 8 : window.innerWidth < 768 ? 10 : window.innerWidth < 1024 ? 12 : 20,
                    maxSize: window.innerWidth < 480 ? 12 : window.innerWidth < 640 ? 16 : window.innerWidth < 768 ? 20 : window.innerWidth < 1024 ? 25 : 40
                },
                pie: {
                    startAngle: 0,
                    states: {
                            ...
                            mouseOver() {
                                ...
                                bubbleSeries.points.forEach(point => {
                                    if (point.custom.tag !== selectedTag) {
                                        ...
                                        if (point.customImageGroup)
                                            point.customImageGroup.attr({
                                                opacity: 0.2
                                            });
                                    }
                                });
                                const matchingCount = bubbleSeries.points.filter(p => p.custom.tag === selectedTag).length;
                                pieSeries.customLabel = fillCenter(matchingCount, this.custom.displayTag.toUpperCase(), chart, pieSeries.customLabel);
                            },
                            mouseOut() {
                                const chart = this.series.chart;
                                const bubbleSeries = chart.series[0];
                    ...
                    point: {
                        events: {
                            mouseOver() {
                                ...
                                series.customLabel = fillCenter(this.options.custom.count, this.options.custom.displayTag.toUpperCase(), chart, series.customLabel);
                            },
                            mouseOut() {
                                const chart = this.series.chart;
                                const bubbleSeries = chart.series[0];
                                ...
                                series.customLabel = fillCenter(processedProjects.size, 'TOTAL PROJECTS', chart, series.customLabel);
                            }
                        }
                    },
                    data: pieData
        ...
        document.getElementById('ecosystem-chart-container').innerHTML = '<div style="color: #ff6961; text-align: center; padding: 50px;">Error loading ecosystem data</div>';
    }
}
function main(ratio) {
